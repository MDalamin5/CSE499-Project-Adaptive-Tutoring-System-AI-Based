...
def setup_step_by_step_llm(language):
    ...
    # Create the prompt template - FIXED: Using dictionary format instead of list
    prompt = ChatPromptTemplate.from_messages([
        ("system", system_prompt),
        MessagesPlaceholder(variable_name="messages")
    ])
...
def create_sentiment_analysis_chain(model, language):
    ...
    # Create the prompt template - FIXED: Using proper format
    prompt = ChatPromptTemplate.from_messages([
        ("system", system_prompt)
    ])
...
def create_adaptive_prompt_generator(model, base_prompt):
    # Setup the prompt generators - FIXED: Using proper format for all templates
    ...
    def generate_adaptive_prompt(input_dict):
        ...
        try:
            if sentiment == "correct":
                output = model.invoke(prompt_correct.format_messages(
                    student_response=student_response,
                    tutor_hint=tutor_hint
                ))
                ...
                output = model.invoke(prompt_partially_correct.format_messages(
                    student_response=student_response,
                    tutor_hint=tutor_hint
                ))
                return process_adaptive_output(output.content)
                ...
                output = model.invoke(prompt_incorrect.format_messages(
                    student_response=student_response,
                    tutor_hint=tutor_hint
                ))
                return process_adaptive_output(output.content)
